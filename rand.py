#!/usr/bin/env python
# -*- coding: utf-8 -*-

__version__ = "0.0.2"

import math


class Generator():
    """Genèrador congruencia lineal multiplicatiu mòdul m primer d'ordre 1
    El llistat de llavors l'ha donat el professor a l'enunciat de la primera
    pràctica.
    Nombres bons:
    m = 2147483647 = 2**31 -1
    a =  630360016 = 2**4 * 11 * 13 * 137 * 2011
    """
    def __init__(self, seed=0):
        self._seed_list = [
            1973272912,281629770, 20006270, 1280689831, 2096730329,
            1933576050, 913566091,246780520, 1363774876, 604901985,
            1511192140, 1259851944, 824064364,150493284, 242708531,
            75253171, 1964472944, 1202299975, 233217322, 1911216000,
            726370533,403498145,993232223, 1103205531, 762430696,
            1922803170, 1385516923, 76271663,413682397,726466604,
            336157058, 1432650381, 1120463904, 595778810, 877722890,
            1046574445, 68911991, 2088367019, 748545416, 622401386,
            2122378830,640690903, 1774806513, 2132545692, 2079249579,
            78130110,852776735, 1187867272, 1351423507, 1645973084,
            1997049139,922510994, 2045512870,898585771, 243649545,
            1004818771,773686062,403188473,372279877, 1901633463,
            498067494, 2087759558,493157915,597104727, 1530940798,
            1814496276, 536444882, 1663153658,855503735, 67784357,
            1432404475,619691088, 119025595,880802310,176192644,
            1116780070,277854671, 1366580350, 1142483975, 2026948561,
            1053920743,786262391, 1792203830, 1494667770, 1923011392,
            1433700034, 1244184613, 1147297105,539712780, 1545929719,
            190641742, 1645390429,264907697,620389253, 1502074852,
            927711160, 364849192, 2049576050,638580085, 547070247,
            ]
        self._nseed = seed % len(self._seed_list)
        self._z = self._seed_list[self._nseed]
        self._m = 2**31 - 1
        self._a = 2**4 * 11 * 13 * 137 * 2011

    @property
    def num(self):
        self._z = (self._a * self._z) % self._m
        return self._z/float(self._m)

    def next_generator(self):
        """Crea un generador amb la següent secció de llavor"""
        return Generator((self._nseed +1) % len(self._seed_list))

    def exp(self, lambd):
        """
        Distribució exponencial de mitja 1/lamd
        http://en.wikipedia.org/wiki/Exponential_distribution
        """
        if lambd <= 0:
            return 0
        x = self.num
        return - lambd * math.log(1 - x)

    def constant(self, x):
        """
        Distribució constant. Escrita per motius de completesa.
        """
        return x

    def uniform(self, inf, sup):
        """Distribució uniforme"""
        assert(inf >= 0)
        assert(sup > 0)
        assert(inf < sup)
        x = self.num
        return x * (sup - inf) + inf


if __name__ == '__main__':
    n = 10
    gen = Generator()
    for i in range(n):
        print gen.num

